...
const AdminSkills = () => {
  ...
  const { skills, loading: skillsLoading, error: skillsError } = useAppSelector((state) => state.skills);
  const { skillsCategories, loading: categoriesLoading, error: categoriesError } = useAppSelector((state) => state.skillsCategory);
  ...
  const [isCategoryDialogOpen, setIsCategoryDialogOpen] = useState(false);
  const [newCategoryName, setNewCategoryName] = useState("");
  const [newCategoryColor, setNewCategoryColor] = useState(predefinedGradientColors[0].value);

  const [isEditingCategory, setIsEditingCategory] = useState(false);
  const [editingCategoryId, setEditingCategoryId] = useState<string | null>(null);
  const [editCategoryName, setEditCategoryName] = useState("");
  const [editCategoryColor, setEditCategoryColor] = useState(predefinedGradientColors[0].value);
  ...
  useEffect(() => {
    if ((skillsCategories && skillsCategories.length > 0) || (!categoriesLoading && skillsCategories?.length === 0)) {
      dispatch(fetchSkills());
    }
  }, [dispatch, skillsCategories, categoriesLoading]);
  ...
  const handleSkillSave = async () => {
    ...
    try {
      if (editingSkillId) {
        await dispatch(updateSkill({ id: editingSkillId, data: editData as Partial<Skill> })).unwrap();
        toast.success("Skill updated successfully!");
      } else {
        await dispatch(addSkill(editData as Omit<Skill, "_id">)).unwrap();
        toast.success("Skill added successfully!");
  ...
  const handleCategorySave = async () => {
    ...
    if (skillsCategories?.some(cat => cat?.category?.toLowerCase() === trimmedName.toLowerCase())) {
      toast.error("Category with this name already exists.");
      return;
    }
    try {
      await dispatch(createSkillsCategory({ category: trimmedName, color: newCategoryColor })).unwrap();
      toast.success("Category added successfully!");
      dispatch(getSkillsCategories());
      setIsCategoryDialogOpen(false);
    } catch (error: any) {
  ...
  const handleEditCategorySave = async () => {
    ...
    if (skillsCategories.some(cat => cat.category.toLowerCase() === trimmedName.toLowerCase() && cat._id !== editingCategoryId)) {
      toast.error("Another category with this name already exists.");
      return;
    }
    if (!editingCategoryId) {
    ...
    try {
      await dispatch(updateSkillsCategory({ id: editingCategoryId, data: { category: trimmedName, color: editCategoryColor } })).unwrap();
      toast.success("Category updated successfully!");
      dispatch(getSkillsCategories());
      setIsEditingCategory(false);
      setEditingCategoryId(null);
  ...
  return (
    <div className="space-y-6 lg:space-y-8 p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Skills Management</h1>
          ...
          <Card key={category._id} className="p-4 lg:p-6 border" hover={false}>
            <div className="flex justify-between items-start gap-3 mb-6">
              <div className="flex items-center gap-3">
                ...
                <div className={`w-6 h-6 rounded-full bg-gradient-to-r ${category.color} ring-1 ring-gray-300`} />
                <h3 className="text-lg lg:text-xl font-bold text-gray-900 capitalize">{category.category}</h3>
                {/* Category actions */}
                <div className="flex gap-1 ml-4 flex-shrink-0">
                  <Button size="sm" variant="outline" onClick={() => handleEditCategory(category._id!)} aria-label={`Edit category ${category.category}`}>
                    <Edit size={12} />
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => handleDeleteCategoryClick(category._id!, category.category)} aria-label={`Delete category ${category.category}`}>
                    <Trash2 size={12} />
                  </Button>
                </div>
              </div>
              <Button onClick={() => handleAddNew(category.category as Skill["category"])} className="w-full sm:w-auto flex-shrink-0">
                <Plus size={16} className="mr-2" /> Add Skill
              </Button>
            </div>
            <div className="space-y-4">
              {(groupedSkills[category.category] || []).length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  <p className="text-sm">No skills in this category yet.</p>
                  <Button size="sm" variant="outline" onClick={() => handleAddNew(category.category as Skill["category"])} className="mt-2">
                    Add First Skill
                  </Button>
                </div>
              ) : (
                (groupedSkills[category.category] || []).map((skill) => (
                  <div key={skill._id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                        <span className="font-medium text-gray-900 truncate">{skill.name}</span>
                        <span className="text-sm text-gray-500 flex-shrink-0">{skill.level}%</span>
                      </div>
                      <ProgressBar value={skill.level} color={extractSolidColor(getCategoryColor(skill.category))} showValue={false} />
                    </div>
                    <div className="flex gap-1 ml-4 flex-shrink-0">
                      <Button size="sm" variant="outline" onClick={() => handleEdit(skill)}>
                        <Edit size={12} />
                      </Button>
                      <Button size="sm" variant="secondary" onClick={() => handleDeleteSkillClick(skill._id, skill.name)}>
                        <Trash2 size={12} />
                      </Button>
                    </div>
                  </div>
      ...
      {/* Skill Dialog */}
      <Dialog open={isSkillDialogOpen} onOpenChange={(open) => !open && closeSkillDialog()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingSkillId ? "Edit Skill" : "Add Skill"}</DialogTitle>
            <DialogDescription>{editingSkillId ? "Update your skill details." : "Enter details for the new skill."}</DialogDescription>
          </DialogHeader>

          <div className="space-y-4 mt-4">
            <div>
              <label htmlFor="skill-name" className="block text-sm font-medium text-gray-700 mb-2">Skill Name</label>
              <input
                id="skill-name"
                type="text"
                value={editData.name || ""}
            ...
            <div>
              <label htmlFor="skill-category" className="block text-sm font-medium text-gray-700 mb-2">Category</label>
              <Select
                value={
                  editData.category ||
                  (skillsCategories.length > 0 ? skillsCategories[0]?.category : "")
                }
                onValueChange={(value) => setEditData({ ...editData, category: value as Skill["category"] })}
              >
                <SelectTrigger
                  id="skill-category"
                  className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                ...
                <SelectContent>
                  {skillsCategories?.length === 0 ? (
                    <SelectItem disabled value="">No categories available. Please add one first.</SelectItem>
                  ) : (
                    skillsCategories?.map((cat) => (
                      <SelectItem key={cat._id} value={cat.category}>
                        <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <input
                ...
                max={100}
                value={editData.level || 0}
                onChange={(e) => setEditData({ ...editData, level: parseInt(e.target.value, 10) })}
      ...
      {/* Add Category Dialog */}
      <Dialog open={isCategoryDialogOpen} onOpenChange={(open) => !open && closeCategoryDialog()}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Add New Category</DialogTitle>
            <DialogDescription>Enter a name and choose a color for the new skill category.</DialogDescription>
          </DialogHeader>

          <div className="mt-4 space-y-4">
            <div>
              <label htmlFor="new-category-name" className="block text-sm font-medium text-gray-700 mb-2">Category Name</label>
              <input
                id="new-category-name"
                type="text"
                value={newCategoryName}
            ...
            <div>
              <label htmlFor="new-category-color" className="block text-sm font-medium text-gray-700 mb-2">Choose Color</label>
              <div className="flex flex-wrap gap-2 p-2 border border-gray-300 rounded-lg">
                {predefinedGradientColors.map(({ value, label }) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full bg-gradient-to-r ${value} border-2 ${newCategoryColor === value ? "border-indigo-500 ring-2 ring-indigo-500" : "border-transparent"}`}
      ...
      {/* Edit Category Dialog */}
      <Dialog open={isEditingCategory} onOpenChange={(open) => !open && setIsEditingCategory(false)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Edit Category</DialogTitle>
          ...
          <div className="mt-4 space-y-4">
            <div>
              <label htmlFor="edit-category-name" className="block text-sm font-medium text-gray-700 mb-2">Category Name</label>
              <input
                id="edit-category-name"
                type="text"
                value={editCategoryName}
            ...
            <div>
              <label htmlFor="edit-category-color" className="block text-sm font-medium text-gray-700 mb-2">Choose Color</label>
              <div className="flex flex-wrap gap-2 p-2 border border-gray-300 rounded-lg">
                {predefinedGradientColors.map(({ value, label }) => (
                  <button
                    key={value}
                    ...
                    className={`w-8 h-8 rounded-full bg-gradient-to-r ${value} border-2 ${editCategoryColor === value ? "border-indigo-500 ring-2 ring-indigo-500" : "border-transparent"}`}
                    aria-label={`Select color ${label}`}
                    title={label}
                  />
                ))}
      ...
      {/* Alert Dialog for Skill Delete */}
      <AlertDialog open={isSkillDeleteDialogOpen} onOpenChange={setIsSkillDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Skill</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete the skill <strong>{skillToDelete?.name}</strong>? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setIsSkillDeleteDialogOpen(false)}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                if (skillToDelete) {
                  try {
      ...
      <AlertDialog open={isCategoryDeleteDialogOpen} onOpenChange={setIsCategoryDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Category</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete the category <strong>{categoryToDelete?.name}</strong>? This will also delete all skills associated with this category.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setIsCategoryDeleteDialogOpen(false)}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                if (categoryToDelete) {
                  try {
                    await dispatch(deleteSkillsCategory(categoryToDelete.id)).unwrap();
                    toast.success(`Category "${categoryToDelete.name}" and its skills deleted successfully!`);
                    dispatch(getSkillsCategories());
                    dispatch(fetchSkills());
                    if (editingCategoryId === categoryToDelete.id) {
                      setIsEditingCategory(false);
